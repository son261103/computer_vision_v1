...
class Visualizer:
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        try:
            ...
            for det in detections:
                ...
                color = tuple(self.vis_config['colors'].get(class_name, [255, 255, 255]))
    ...
    def draw_status(self, frame: np.ndarray, status: str,
        ...
        try:
            ...
            # Calculate text size
            (text_w, text_h), _ = cv2.getTextSize(
                status, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
    ...
    def create_info_panel(self, frame: np.ndarray, info: Dict) -> np.ndarray:
        """Create enhanced information panel with sections"""
        try:
            # Panel configuration
            panel_height = 100
            padding = 10
